...
class ModelId(BaseModel):
    ...
    config_template: str = Field(description="Config template for the model.")
    ...
    def to_compressed_str(self) -> str:
        ...
        return f"{self.namespace}:{self.name}:{self.config_template}:{self.hotkey}:{self.hash}:{self.competition_id}"
    ...
    def from_compressed_str(cls, cs: str) -> Type["ModelId"]:
        ...
        return cls(
            namespace=tokens[0],
            name=tokens[1],
            config_template=tokens[2] if tokens[2] != "None" else None,
